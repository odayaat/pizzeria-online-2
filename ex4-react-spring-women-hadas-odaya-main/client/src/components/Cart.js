    ...
    return (
        <Wrapper>
            <div>
                ...
                <h1>Your cart 🛒</h1>
                <div className="col card bg-light p-3">
                    <h2>Pizzas 🍕 List:</h2>
                    {addedpizza.map((pizza, index) => (
                        <div className="card p-2 mb-2 d-flex flex-row justify-content-between align-items-center" key={index} style={{ minHeight: '80px' }}>
                            <div>
                            ...
                            <div className="d-flex">
                                <button onClick={() => editIngredient(pizza, index)} data-bs-toggle="modal" data-bs-target="#editModal" className="btn btn-primary me-2">Edit</button>
                                <button className="btn btn-danger" onClick={() => deletePizza(index)}>Delete</button>
                            </div>
                        </div>
                    ))}
                    <div>Price: {pricepizza}</div>
                    <button className="btn btn-primary mt-3" onClick={buyPizza}>Buy</button>
                </div>
            </div>
                                ...
                                <form>
                                    <div className="d-flex flex-wrap">
                                        {ingredients.map((ingredient, index) => (
                                            <div key={index} className="card p-2 m-2" style={{ width: '120px', height: '100px', backgroundColor: 'transparent', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                                                <label className="form-check-label d-flex flex-column align-items-center">
                                                    <input
                                                    ...
                                                    />
                                                    <img src={ingredient.image} alt={ingredient.nameProduct} style={{ width: '50px', height: '50px', marginBottom: '10px' }} />
                                                    <span>{ingredient.nameProduct}</span>
                                                </label>
                                            </div>
                                        ))}
                                    </div>
